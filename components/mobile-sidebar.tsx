...
export function MobileSidebar({ isOpen, onClose }: MobileSidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] backdrop-empire border-l border-amber-500/20 z-50 md:hidden transform transition-transform duration-300 ease-in-out ${
        ...
        <div className="flex items-center justify-between p-6 border-b border-amber-500/20">
          <div className="flex items-center space-x-3 group">
            ...
            <div className="transform transition-all duration-300 group-hover:translate-x-1">
              <h1 className="text-lg font-bold text-empire-primary bg-gradient-to-r from-current to-amber-200 bg-clip-text text-transparent">
          ...
          <Button
            ...
            size="sm"
            onClick={onClose}
            className="text-empire-primary hover:text-amber-400 hover:bg-amber-400/10 transition-all duration-300 p-2"
            ...
            {NAVIGATION_DATA.items.map((item, index) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center space-x-4 px-4 py-3 rounded-lg text-empire-primary hover:text-amber-400 hover:bg-amber-400/10 transition-all duration-300 group relative overflow-hidden"
                  style={{
        ...
        <div className="px-6 py-4 border-t border-amber-500/20 space-y-4">
          ...
          <div className="flex items-center justify-between">
            <span className="text-empire-muted text-sm">
              {language === "zh" ? "主题" : language === "ja" ? "テーマ" : "Theme"}
            </span>
            <ThemeToggle />
          </div>
